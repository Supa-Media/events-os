 ...
 *  all it is for. Markup that nobody reads is stripped before counting: HTML
 *  tags (posts embed raw tables/callouts, and `<td>` is not a word) and
 *  markdown link targets (`[text](url)` → the text is read, the URL is not).

 ...
 *
 * Personal-repayment note: the Phase-5 card contract has no query that lists a
 * member's pending repayments, so a flagged charge is tracked in local state for
 * the session it was flagged in. Flag → `flagPersonalCharge` (returns the
 * repayment) → "Pay back $X" → `markRepaymentPaid` (posts the offsetting credit).
 */
...
export function MemberCardsView() {
  ...
  const payBack = useMutation(api.cards.markRepaymentPaid);
  ...
  const pending = useMemo(
    () => Object.values(repayments).filter((r) => r.status !== "paid"),
    [repayments],
  );
  const owedCents = pending.reduce((sum, r) => sum + r.amountCents, 0);
  ...
  async function handlePayBack(repaymentId: string, transactionId: string) {
    const res = await run(
      () => payBack({ repaymentId: repaymentId as Id<"personalRepayments"> }),
      { errorTitle: "Couldn't record repayment" },
    );
    if (res) setRepayments((m) => ({ ...m, [transactionId]: res }));
  }

  async function payAllPending() {
    for (const r of pending) {
      await handlePayBack(r.id, r.transactionId);
    }
  }
  ...
  return (
    <View>
      ...
      {/* Personal-repayment banner (a charge flagged this session). */}
      {pending.length > 0 ? (
        <View
            ...
            <View className="flex-1 flex-row items-start gap-3">
              ...
              <View className="flex-1">
                ...
                <Text className="text-xs text-muted">
                  {pending.length} charge{pending.length === 1 ? "" : "s"} flagged
                  personal. Pay it back from your own debit card or bank (ACH) — it
                  posts an offsetting credit, no reimbursement paperwork.
                </Text>
              </View>
            </View>
            <Button
              title={`Pay back ${formatCents(owedCents)}`}
              size="sm"
          ...
          {charges.map((t, i) => {
            ...
            return (
              ...
              >
                ...
                <View className="w-[136px] items-end">
                  {rep && rep.status === "paid" ? (
                    ...
                    <Button
                      ...
                      size="sm"
                      icon="refresh-cw"
                      onPress={() => handlePayBack(rep.id, t.id)}
                    />
                  ) : (
                    <Button
                      title="Flag personal"

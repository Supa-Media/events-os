/**
 * BLOCKS DOCUMENT COMPOSER — the block-based email editor, in one place.
 *
 * Renamed from `DocumentComposer` (2026-07-29, the maily editor overhaul —
 * `docs/plans/maily-editor-overhaul.md`, WS3): `DocumentComposer.tsx` is now
 * the FORMAT SWITCH (`composerHostForFormat`, `@events-os/shared`'s
 * `emailDocFormatOf`) that routes a `"blocks"`-format row here, UNTOUCHED,
 * and a `"tiptap"`-format row to `MailyDocumentHost` instead. Nothing in this
 * file changed beyond the component/type names — every block-format
 * campaign/template still gets exactly this canvas, forever (see the format
 * switch's own doc for why old docs never convert in place).
 *
...
export type BlocksDocumentComposerProps = {
...
export function BlocksDocumentComposer({
  doc: loadedDoc,
  editable,
  lockedNotice,
  onSave,

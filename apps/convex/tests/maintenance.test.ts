 ...
 * #161 cardDetailsRevealAttempts) — both only ever INSERT, so a daily cron
 * (crons.ts) sweeps rows older than the 1-hour rate window.
 */
describe("maintenance.sweepRateLimitAttempts", () => {
  test("drops attempt rows older than the 1-hour window, keeps recent ones, across both tables", async () => {
    const t = newT();
    const now = Date.now();
    ...
    expect(result).toEqual({ reimbursementAttempts: 1, cardRevealAttempts: 1 });
  ...
  test("no-ops cleanly when both tables are empty", async () => {
    const t = newT();
    const result = await t.mutation(internal.maintenance.sweepRateLimitAttempts, {});
    expect(result).toEqual({ reimbursementAttempts: 0, cardRevealAttempts: 0 });
  });
});

 ...
 * opportunistically fixing a stale `aiSuggestion.fundId` on the same row too
 * — that field is advisory-only, cleared the moment a human categorizes, so
 * a full separate scan for it isn't warranted; `reimbursementLineItems`;
  ...
  for (const extra of extras) {
    ...
    for (const tr of transactions) {
      await ctx.db.patch(tr._id, {
        fundId: keeperId,
        // Opportunistic: fix a stale AI suggestion on the SAME row while
        // we're already here (see docstring — not a separate scan).

 ...
 *
 * Owner decision (2026-07-17): Chapter Director is ONE role with identical
 * expectations across every chapter — the derived mirror is not a second duty
 * target. This backfill repoints every `responsibilities` row's
 * `assigneeSeatIds`: any entry equal to the derived seat's id is replaced
 * with the real chapter-chart root seat's id, deduping if a row already had
 * both (matching via `responsibilityAppliesTo` is a plain Set-membership
 * check, so a duplicate entry wouldn't have changed WHO a duty applied to,
 * but it's debris no surface should keep rendering post-repoint).
 *

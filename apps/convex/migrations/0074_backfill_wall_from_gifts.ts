...
/**
 * Give the public giving wall its history, so "every gift, in public" is true
 * on the day it ships.
 *
 ...
 * under the headline "Every gift, in public" would open showing a handful of
 * rows from whenever someone last filled in the optional box — which is worse
 * than the old wall, because the new copy makes a claim the data doesn't
 * support.
 *
 ...
 *    shadowed by an anonymous duplicate;
 *  - this backfill's own key, `gift:<giftId>`, for everything else.
...
export async function runBackfillWallFromGifts(ctx: MutationCtx): Promise<{
  ...
  for (const gift of oldestFirst) {
    ...
    // name) and must not be duplicated anonymously beside itself.
    if (gift.externalRef) {
      const live = await ctx.db
        .query("givingActivity")
        .withIndex("by_refKey", (q) => q.eq("refKey", gift.externalRef!))
        .unique();
      if (live) {
        skippedExisting += 1;
        continue;

 ...
 *
 * AND A SECOND EMAIL WHEN THE MONEY ACTUALLY GOES (2026-08-14, same
 * conversation; founder: "let's also send emails when we actually pay people —
 * this is really important to them. Email when approved and email when paid").
 * A claimant now hears from us TWICE about one request, and the note above
 * teaches only the first, which would leave the lesson quietly wrong about what
 * to expect. Same lesson carries it, content-only, minutes stay 5.
 *

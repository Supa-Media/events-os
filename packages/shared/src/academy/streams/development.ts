...
export const DEVELOPMENT_SECTIONS: Omit<AcademySection, "order">[] = [
  ...
  // ── 88 · Donor stewardship: the canonical import ────────────────────────
  {
    ...
    blocks: [
      ...
      {
        kind: "p",
        text: "You usually don't even have to build rowType columns yourself — the paste box recognizes Givebutter's own exports. Paste a Givebutter TRANSACTIONS export and it asks you to map each campaign to gift / ticket / skip, then classifies every row for you. Paste a Givebutter CONTACTS export (the full mailing list) and every row becomes a `contact` — first and last names kept as Givebutter had them, existing people matched by email, phone, or exact name (never duplicated), and no donors or money history created from it, ever. That's how the whole newsletter list comes in: contacts import reachable, and anyone who unsubscribes from a Public Worship email is honored by our own suppression list from then on, no matter what the source platform said.",
      },
      {
        kind: "rule",

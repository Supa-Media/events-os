  ...
  // ── 87 · Donor stewardship: the relationship workflow ───────────────────
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        text: "Building a campaign's audience isn't a fixed source dropdown anymore — it's robust filter chips (giving amount, backer status, attended-event, role, team-vs-contact, chapter, verified email) that AND-combine, plus an optional hand-picked list you build by searching for people by name or email. Both can be used together: filters catch everyone who matches a pattern, hand-picks add (or remove) specific individuals the filters wouldn't otherwise catch. Filters can also EXCLUDE — a second \"Exclude people who…\" set of the same criteria removes anyone matching, even someone you hand-picked into the include list (an explicit exclusion always beats a manual include). Audiences are LIVE — someone who starts matching the criteria tomorrow (a new donor crosses the giving threshold, a guest RSVPs to a qualifying event) is included automatically the next time the audience is used, with no re-save needed, and the same is true of exclusions: someone who starts matching an exclude filter later is dropped automatically too. One thing hand-picking can NEVER do, on either side: override consent. If a person has unsubscribed (their address is suppressed) or opted out of marketing, they're excluded even if you explicitly added them to the include list — the composer's live preview always shows exactly who was dropped and why.",
      },
    ],
    quiz: [
      {
